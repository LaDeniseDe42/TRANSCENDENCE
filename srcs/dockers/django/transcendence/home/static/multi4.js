function loadTabM4() {
    ...
    function Ball(pos, velocity, radius) {
        ...
        this.update = function () {
            ...
            if (this.pos.y + this.velocity.y < gameBoard.height && this.pos.y + this.velocity.y > 0) {
                this.pos.y += this.velocity.y;
            } else if (this.pos.y + this.velocity.y <= 0) {
                this.pos.y = 0;
            } else if (this.pos.y + this.velocity.y >= gameBoard.height) {
                this.pos.y = gameBoard.height;
            }
        };
        this.draw = function () {
            ctx.fillStyle = "#fff";
    ...
    function Paddle(pos, velocity, width, height) {
        ...
        this.update = function () {
            if (keysPressed['w'])
                paddle1.pos.y -= paddle1.velocity.y / 4;
            if (keysPressed['s'])
                paddle1.pos.y += paddle1.velocity.y / 4;
            if (keysPressed['ArrowUp'])
                paddle4.pos.y -= paddle2.velocity.y / 4;
            if (keysPressed['ArrowDown'])
                paddle4.pos.y += paddle2.velocity.y / 4;
            if (keysPressed['g'])//lettre g
                paddle3.pos.y -= paddle3.velocity.y / 4;
            if (keysPressed['b'])//lettre b
                paddle3.pos.y += paddle3.velocity.y / 4;
            if (keysPressed['6'])//pavé numérique6
                paddle2.pos.y -= paddle4.velocity.y / 4;
            if (keysPressed['3'])//pavé numérique3
                paddle2.pos.y += paddle4.velocity.y / 4;
        }
        this.draw = function () {
            ctx.fillStyle = "#fff";
    ...
    const paddle2 = new Paddle(vec2(gameBoard.width - paddleMargin - 20, 50), vec2(5, 5), 20, 160);
    const paddle3 = new Paddle(vec2(400, 50), vec2(5, 5), 20, 80);
    const paddle4 = new Paddle(vec2(gameBoard.width - 400, 50), vec2(5, 5), 20, 80);
    let paddle1VelocityChange = vec2(10, 10);
    let paddle2VelocityChange = vec2(10, 10);
    let paddle3VelocityChange = vec2(10, 10);
    let paddle4VelocityChange = vec2(10, 10);
    ...
    decreasePaddlePace.addEventListener('click', function () {
        if ((gameOver || !gameStarted) && (paddleVelocity > 1)) {
            ...
            paddle3VelocityChange.x -= 1;
            paddle3VelocityChange.y -= 1;
            paddle4VelocityChange.x -= 1;
            paddle4VelocityChange.y -= 1;
            paddleVelocity--;
    ...
    function gameUpdate() {
        ...
        paddleCollisionWithTheEdges(paddle3);
        paddleCollisionWithTheEdges(paddle4);
        ballCollisionWithTheEdges(ball);
        ballPaddleCollision(ball, paddle1);
        ballPaddleCollision(ball, paddle2);
        ballPaddleCollision(ball, paddle3);
        ballPaddleCollision(ball, paddle4);
        GameEvent();
    ...
    function showGameOverModal() {
        if (window.location.pathname.includes("game/multi4")) {
    ...
    function startGame() {
        ...
        paddle2.pos = vec2(gameBoard.width - paddleMargin - 20, 50);
        paddle3.pos = vec2(400, 50);
        paddle4.pos = vec2(gameBoard.width - paddleMargin - 400, 50);
        ...
        paddle3.velocity.x = paddle3VelocityChange.x;
        paddle3.velocity.y = paddle3VelocityChange.y;
    ...
    // Add this inside the loadTabM function
    ...
    function restartGame() {
        ...
        paddle3.velocity.x = paddle3VelocityChange.x;
        paddle3.velocity.y = paddle3VelocityChange.y;
        paddle4.velocity.x = paddle4VelocityChange.x;
        paddle4.velocity.y = paddle4VelocityChange.y;
        paddlePaceValue.textContent = paddleVelocity;
